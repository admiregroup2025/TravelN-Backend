...
export async function createItinerary(req, res) {
	try {
		const { title, slug, type, country, city, shortDescription, coverImageUrl, gallery, priceFrom, durationDays, inclusions, exclusions, dayPlans } = req.body;
		if (!title || !slug || !type) return res.status(400).json({ message: "title, slug, type are required" });
